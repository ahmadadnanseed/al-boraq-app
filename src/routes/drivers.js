...
router.post("/driver/signup", (req, res) => {
  ...
  const getUserSql = `SELECT * FROM customer WHERE caustomer_id = ?`;
  ...
  connection.query(getUserSql, [userId], (err, userResult) => {
    ...
    const checkDriverSql = `SELECT * FROM driver WHERE phone_driver = ?`;
      ...
      connection.query(
        ...
        (driverErr, driverInsertResult) => {
          if (driverErr) {
            ...
            return res.json({ success: false, message: "فشل حفظ بيانات السائق" });
          }
          ...
          connection.query(
            vehicleSql,
            [vehicleType, vehicleModel, vehicleYear, plateNumber, driverId, color],
            (vehicleErr) => {
              if (vehicleErr) {
                console.log("Vehicle insert error:", vehicleErr);
                return res.json({ success: false, message: "فشل حفظ بيانات المركبة" });
              }
...
router.get("/driver/profile-by-id/:driverId", (req, res) => {
  ...
  connection.query(sql, [driverId], (err, result) => {
    if (err) {
      ...
      return res.json({ success: false, message: "فشل جلب بيانات السائق" });
    }
    ...
    res.json({ success: true, driver: result[0] });
  });
});

router.put("/driver/profile/:driverId", (req, res) => {
  const driverId = req.params.driverId;
  const { name, phone, address, vehicleType, vehicleModel, vehicleYear, plate, color } = req.body;
  const { first: firstName, last: lastName } = splitFullName(name);

  const updateDriverSql = `
    UPDATE driver
    SET fast_name_driver = ?, last_name_driver = ?, phone_driver = ?, address = ?
    WHERE driver_id = ?
  `;

  connection.query(updateDriverSql, [firstName, lastName, phone, address, driverId], (driverErr) => {
    if (driverErr) {
    ...
    const updateVehicleSql = `
      UPDATE vehicle
      SET vehicle_type = ?, vehicle_mode = ?, vehicle_year_of_manufacturel = ?, vehicle_license_blate_number = ?, color = ?
      WHERE driver_id_fk = ?
    `;

    connection.query(updateVehicleSql, [vehicleType, vehicleModel, vehicleYear, plate, color, driverId], (vehicleErr) => {
      if (vehicleErr) {
...
router.get("/driver/booking-requests", (req, res) => {
  const sql = `
    SELECT
      b.booking_id, b.start_date_booking, TIME_FORMAT(b.start_time, '%H:%i') AS start_time,
      TIME_FORMAT(b.end_time, '%H:%i') AS end_time, b.status, b.trip_type, b.pickup, b.dropoff,
      b.seats_count, b.selected_seats, b.price, b.distance_km, b.duration_min,
      c.caustomer_id, c.fast_name_caustomer, c.last_name_caustomer, c.date_of_birth_caustomer, c.phone_caustomer,
      d.dependent_id, d.fast_name_dependent, d.last_name_dependent, d.relationship, d.phone AS dependent_phone, d.date_of_birth_dependent
    FROM booking b
...
router.put("/driver/booking-requests/:id/accept", (req, res) => {
  ...
  const sql = `UPDATE booking SET driver_id_fk = ?, status = 2 WHERE booking_id = ? AND status = 1`;
  ...
  connection.query(sql, [driverId, bookingId], (err, result) => {
    ...
    createDailyTripsFromBooking(bookingId, (tripErr) => {
      if (tripErr) {
        ...
        return res.json({ success: false, message: "تم قبول الطلب لكن فشل إنشاء الرحلات" });
      }
      res.json({ success: true, message: "تم قبول الطلب وإنشاء الرحلات اليومية" });
    });
  });
});
...
router.get("/driver/today-trips/:driverId", (req, res) => {
  ...
  const sql = `
    SELECT
      ...
      c.fast_name_caustomer, c.last_name_caustomer, c.phone_caustomer, c.date_of_birth_caustomer,
      d.fast_name_dependent, d.last_name_dependent, d.relationship
    FROM daily_trips dt
    JOIN customer c ON dt.customer_id_fk = c.caustomer_id
    LEFT JOIN dependent d ON dt.dependent_id_fk = d.dependent_id
    WHERE dt.driver_id_fk = ? AND dt.trip_date = CURDATE()
...
router.put("/driver/daily-trips/:tripId/arrived", (req, res) => {
  ...
  const sql = `UPDATE daily_trips SET status = 'driver_arrived', driver_arrival_confirmed = 1 WHERE daily_trip_id = ?`;
...
router.put("/driver/daily-trips/:tripId/completed", (req, res) => {
  const tripId = req.params.tripId;
  const sql = `UPDATE daily_trips SET status = 'completed' WHERE daily_trip_id = ?`;
...
router.put("/driver/daily-trips/:tripId/start", (req, res) => {
  const tripId = req.params.tripId;
  const sql = `UPDATE daily_trips SET status = 'driver_started' WHERE daily_trip_id = ?`;
...
router.put("/driver/daily-trips/:tripId/picked", (req, res) => {
  const tripId = req.params.tripId;
  const sql = `UPDATE daily_trips SET status = 'passenger_picked' WHERE daily_trip_id = ?`;
...
router.get("/driver/statistics/:driverId", (req, res) => {
  ...
  connection.query(sql, [driverId], (err, result) => {
    if (err) {
      ...
      return res.json({ success: false, message: "فشل تحميل إحصائيات السائق", error: err.sqlMessage });
    }
    res.json({ success: true, statistics: result[0] });
  });
});
